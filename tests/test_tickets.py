...
def test_view_ticket_details(setup_ticket, set_up):
    ...
    expect(case_view.header.client_name).to_contain_text(Config.CLIENT_NAME)
    # - Verificar que el ID en el encabezado coincida con 9266.
    expect(case_view.header.id_label).to_contain_text(f"{ticket_identifier}")
    # - Confirmar que aparezca el icono de WhatsApp como canal de origen.
    expect(case_view.footer_root).to_be_visible(timeout=10000)
    expect(case_view.footer.account_selector).to_contain_text(Config.ACCOUNT_NAME)
    # - Validar que el número de contacto sea 573115734967.
    expect(case_view.header.client_phone).to_contain_text(Config.PHONE)
    # 5. Validación de Contenido (Chat):
    # - Confirmar la presencia del mensaje: "Cordial saludo estimado usuario...".
    expect(case_view.chat.last_message).to_contain_text(Config.CONTAIN_SIMPLE_HSM_TEXT)
